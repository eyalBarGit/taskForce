...
export function CardCheckList({ currCheckList, currCard, currBoard }) {
  ...
  const onRemoveCheckList = () => {
    dispatch(removeCheckList(currBoard, currCard, currCheckList));
  };
  ...
  const onRemoveCheckListItem = useCallback(
    ...
    [currBoard, currCard, currCheckList, dispatch, checkedItems, updatePercentageOnMount,]
  );
  ...
  return (
    <div className="card-check-list">
      <div className="card-check-list-section flex space-between">
        ...
        <p className="delete-checklist" onClick={onRemoveCheckList}>
          Delete
        </p>
      </div>
      ...
      <div className="check-list-action">
        {isAddItemVisible && (
          <ActionForm
            currRef={itemName}
            toggleAdd={onToggleItemVisibility}
            onSubmitFunc={onAddCheckItem}
            name={"listItem"}
